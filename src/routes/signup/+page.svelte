<script>
    import SignupHeader from "./SignupHeader.svelte";
	...
	async function sendVerificationCode() {
		...
		emailVerified = false; // 인증을 다시 시도하므로 false로 설정
	}

	function verifyEmailCode() {
		// 이메일 인증 코드가 '123456'인 경우 성공으로 처리
	...
	async function submitForm() {
		...
		const response = await fetch("https://api.kicketing.com/signup", {
			method: "POST",
			headers: {
...
<div class="container">
	<form on:submit|preventDefault={submitForm}>
		...
		<div>
			<button type="button" on:click={sendVerificationCode}
				>인증번호 보내기</button
			>
		</div>
		...
		{#if showEmailVerification}
			<div>
				...
				/>
				<button type="button" on:click={verifyEmailCode}>확인</button>
				<button type="button" on:click={sendVerificationCode}
					>재전송</button
				>
